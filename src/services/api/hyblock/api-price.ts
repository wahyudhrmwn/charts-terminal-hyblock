...
export const getBidAsk = async (
  ...
  try {
    ...
    let url = `${API_BASE_URL}${HYBLOCK_ENDPOINTS.BID_ASK}?coin=${params.coin}&timeframe=${params.timeframe}`;
    ...
    if (params.depth) url += `&depth=${params.depth}`;
    ...
    console.error('Error fetching bid/ask data:', 
    ...
    return {
      ...
      error: typeof axiosError.response?.data === 'object' && axiosError.response?.data && 'message' in axiosError.response.data
        ...
        : axiosError.message || 'Gagal mendapatkan data bid/ask',
